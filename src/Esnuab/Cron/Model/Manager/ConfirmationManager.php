...
class ConfirmationManager
{
    ...
    public function getConfirmed(Application $app)
    {
        $query = 'SELECT id,userId,name,email,expires_at from socio_confirmation WHERE confirmed = 1 AND facturado_flag = 0';
    ...
    public function trackPdfGenerated(Application $app,$confirmed)
    {
        foreach ($confirmed as $value) {
            $app['db']->update(
                'socio_confirmation',
                array('facturado_flag' => 1),
                array('id' => $value['id'])
            );
        }
    }
